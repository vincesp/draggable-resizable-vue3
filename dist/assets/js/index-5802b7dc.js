...
const _hoisted_1$1 = ["onMousedown", "onTouchstart"];
const _sfc_main$1 = {
  __name: "DraggableResizableVue3",
  props: {
    className: {
      type: String,
      default: "drv"
    ...
    handles: {
      ...
      validator: (val) => {
        const s = /* @__PURE__ */ new Set(["tl", "tm", "tr", "mr", "br", "bm", "bl", "ml"]);
  ...
  setup(__props, { emit }) {
    ...
    const getParentSize = () => {
      if (props.parent) {
        const parent = typeof props.parent === "string" ? el.value.closest(props.parent) : el.value.parentNode;
        parentEl.value = parent;
        const style2 = window.getComputedStyle(parent, null);
        return [
          parseInt(style2.getPropertyValue("width"), 10),
          parseInt(style2.getPropertyValue("height"), 10)
        ];
      }
      return [null, null];
    };
    ...
    const elementDown = (e) => {
      ...
      if (el.value.contains(target)) {
        ...
        if (props.dragHandle && !matchesSelectorToParentElements(target, props.dragHandle, el.value) || props.dragCancel && matchesSelectorToParentElements(target, props.dragCancel, el.value)) {
    ...
    const handleTouchDown = (handle2, e) => {
      eventsFor = events.touch;
      handleDown(handle2, e);
    };
    ...
    const calcResizeLimits = () => {
      ...
      if (props.parent) {
        ...
        limits.maxLeft = left.value + Math.floor((width.value - minW.value) / gridX) * gridX;
        limits.minTop = top.value % gridY;
        limits.maxTop = top.value + Math.floor((height.value - minH.value) / gridY) * gridY;
        limits.minRight = right.value % gridX;
        limits.maxright = right.value + Math.floor((width.value - minW.value) / gridX) * gridX;
        limits.minBottom = bottom.value % gridY;
        limits.maxbottom = bottom.value + Math.floor((height.value - minH.value) / gridY) * gridY;
        if (maxW.value) {
    ...
    const handleResize = (e) => {
      ...
      const tmpDeltaX = mouseClickPosition.value.mouseX - (e.touches ? e.touches[0].pageX : e.pageX);
      const tmpDeltaY = mouseClickPosition.value.mouseY - (e.touches ? e.touches[0].pageY : e.pageY);
      ...
      if (props.onResize(handle.value, leftPx, topPx, widthPx, heightPx) === false) {
        return;
      }
      left.value = leftPx;
      top.value = topPx;
    ...
    onMounted(() => {
      ...
      aspectFactor.value = (width.value !== "auto" ? width.value : widthPx) / (height.value !== "auto" ? height.value : heightPx);
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", {
        ...
        class: normalizeClass([
          {
            [__props.classNameActive]: unref(active) || __props.alwaysActive,
            [__props.classNameDragging]: dragging.value,
            [__props.classNameResizing]: resizing.value,
            [__props.classNameDraggable]: __props.draggable,
            [__props.classNameResizable]: __props.resizable
          },
          __props.className
        ]),
        ...
        (openBlock(true), createElementBlock(Fragment, null, renderList(unref(actualHandles), (handleEl) => {
          return openBlock(), createElementBlock("div", {
            key: handleEl,
            class: normalizeClass([__props.classNameHandle, __props.classNameHandle + "-" + handleEl]),
            style: normalizeStyle({ display: unref(active) ? "block" : "none" }),
            onMousedown: withModifiers(($event) => handleDown(handleEl, $event), ["stop", "prevent"]),
            onTouchstart: withModifiers(($event) => handleTouchDown(handleEl, $event), ["stop", "prevent"])
          }, [
            renderSlot(_ctx.$slots, handle.value)
          ], 46, _hoisted_1$1);
        }), 128)),
...
const DraggableResizableVue3 = "";
const _hoisted_1 = /* @__PURE__ */ createBaseVNode("div", {
  id: "top",
  class: "container"
}, null, -1);
const _hoisted_2 = { style: { "background": "aqua", "width": "771px", "height": "500px" } };
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const position2 = ref({ x: 100, y: 100, w: 100, h: 100, active: true });
    return (_ctx, _cache) => {
        ...
        createBaseVNode("div", null, [
          createBaseVNode("div", _hoisted_2, [
            createVNode(unref(_sfc_main$1), {
              x: position.value.x,
              "onUpdate:x": _cache[0] || (_cache[0] = ($event) => position.value.x = $event),
              y: position.value.y,
              "onUpdate:y": _cache[1] || (_cache[1] = ($event) => position.value.y = $event),
              h: position.value.h,
              "onUpdate:h": _cache[2] || (_cache[2] = ($event) => position.value.h = $event),
              w: position.value.w,
              "onUpdate:w": _cache[3] || (_cache[3] = ($event) => position.value.w = $event),
              active: position.value.active,
              "onUpdate:active": _cache[4] || (_cache[4] = ($event) => position.value.active = $event),
              "show-grid": true,
              grid: [20, 40],
              parent: true,
              "drag-handle": "#handle"
            ...
            createVNode(unref(_sfc_main$1), {
              x: position2.value.x,
              "onUpdate:x": _cache[5] || (_cache[5] = ($event) => position2.value.x = $event),
              y: position2.value.y,
              "onUpdate:y": _cache[6] || (_cache[6] = ($event) => position2.value.y = $event),
              h: position2.value.h,
              "onUpdate:h": _cache[7] || (_cache[7] = ($event) => position2.value.h = $event),
              w: position2.value.w,
              "onUpdate:w": _cache[8] || (_cache[8] = ($event) => position2.value.w = $event),
              active: position2.value.active,
              "onUpdate:active": _cache[9] || (_cache[9] = ($event) => position2.value.active = $event),
              parent: true,
              "drag-handle": "#handle1"
            }, {
              default: withCtx(() => [
